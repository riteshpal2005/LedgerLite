...
import {
  toggleShowIcons,
  toggleHaptics,
  toggle24HourFormat } from
"../../store/settingsSlice";
import { Ionicons } from "@expo/vector-icons";
import { useTheme, ThemeOption } from "../../hooks/theme/ThemeContext";
...
export function PreferencesSection() {
  ...
  const CustomToggle = ({
    ...
    return (
      <Pressable
        ...
        <View
          className={`w-4 h-4 rounded-full bg-white shadow-sm ${value ? "self-end" : "self-start"}`} />
  ...
  const ThemeButton = ({
  ...
  }: {title: string;value: ThemeOption;}) => {
    ...
    return (
      <Pressable
        ...
        <Text
          className={`${isActive ? "text-white" : "text-secondary"} font-bold text-xs`}>
  ...
  return (
    <>
      <Text className="text-secondary font-bold mb-2 uppercase text-xs tracking-wider">
        Preferences
      </Text>
      <View className="bg-surface rounded-2xl p-4 border border-bordercolor">
        <View className="mb-4">
          <Text className="text-primary text-lg font-semibold mb-3">
            App Theme
          </Text>
          <ScrollView
            horizontal
